...
export class ResidencesComponentComponent {
  listResidences:Residence[]=[
    ...
    list:any[]=[];
    listFavoris: any[] = [];
    showAppartmentsClicked = false;
    checkAppart(id:number){
      this.showAppartmentsClicked = true;
    ...
    likeAppart(i: number) {
           ...
           this.listFavoris=this.listApartments.filter((appart)=>appart.id==i);
...
filterApartments() {
  if (this.surface !== null) {
    this.list = this.listApartments.filter((apartment) => apartment.surface === this.surface);
  }
}
  }
